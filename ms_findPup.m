...
%                         fprintf('#%.0f:relation Area/FilledArea %0.3f; Area %.0f FOUND!\n',t, s_old.Area/s_old.ConvexArea, s_old.Area)
%                         break; 
%                     end
%                 end
%             end
...
if(nargin==1)
    % if it's too big
    cands([cands.ConvexArea]>10000) = [];
else
    % if it's too far away
    for ix=1:length(cands); xr(ix)=cands(ix).Centroid(1); yr(ix)=cands(ix).Centroid(2); end
...
% [~,idx]=sort((1-[cands.Eccentricity]) + ([cands.Area]./[cands.ConvexArea]) );
try
    s_old=cands(idx(end));
    fprintf('relation Area/ConvexArea %0.3f; Area %.0f CHOSEN#%.0f of %.0f\n',s_old.Area/s_old.ConvexArea, s_old.Area,idx(end), t)
catch % as a fallback
    s_old=fallBack;
...
s = regionprops(A,{...
    ...
    'MajorAxisLength',...
    'MinorAxisLength',...
    'Orientation', 'Area', 'ConvexArea', 'ConvexImage', 'Eccentricity'});
...
if(~isempty(s)); s.PCNN=A; s.EllArea = s.MajorAxisLength*s.MinorAxisLength*pi/4; end
end
